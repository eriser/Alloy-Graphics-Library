...
#include "../../include/example/stb_image_write.h"
...
void drawEyes(NVGcontext* vg, float x, float y, float w, float h, float mx,
	...
	gloss = nvgRadialGradient(vg, lx - ex * 0.25f, ly - ey * 0.5f, ex * 0.1f,
			ex * 0.75f, nvgRGBA(255, 255, 255, 128), nvgRGBA(255, 255, 255, 0));
	nvgBeginPath(vg);
	nvgEllipse(vg, lx, ly, ex, ey);
...
void drawGraph(NVGcontext* vg, float x, float y, float w, float h, float t) {
	...
	for (i = 1; i < 6; i++)
		nvgBezierTo(vg, sx[i - 1] + dx * 0.5f, sy[i - 1], sx[i] - dx * 0.5f,
				sy[i], sx[i], sy[i]);
...
int loadDemoData(NVGcontext* vg, DemoData* data) {
	...
	data->fontNormal = nvgCreateFont(vg, "sans",
			"../example/Roboto-Regular.ttf");
	if (data->fontNormal == -1) {
		printf("Could not add font italic.\n");
		return -1;
	}
	data->fontBold = nvgCreateFont(vg, "sans-bold",
	...
	while ((nrows = nvgTextBreakLines(vg, start, end, width, rows, 3))) {
		for (i = 0; i < nrows; i++) {
			...
			if (hit) {
				...
				px = x;
				nglyphs = nvgTextGlyphPositions(vg, x, y, row->start, row->end,
						glyphs, 100);
				for (j = 0; j < nglyphs; j++) {
					float x0 = glyphs[j].x;
					float x1 =
							(j + 1 < nglyphs) ?
									glyphs[j + 1].x : x + row->width;
