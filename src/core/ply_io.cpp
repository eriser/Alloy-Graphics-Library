...
PlyFile *open_for_writing_ply(const char *filename, int nelems,
 ...
 */
{
	...
	if (elem->nprops == 0) {
		...
		elem->store_prop = (char *) myrealloc(elem->store_prop,
...
PlyOtherElems *get_other_element_ply(PlyFile *plyfile) {
	...
	if (plyfile->other_elems == NULL) {
		...
		other_elems = plyfile->other_elems;
		other_elems->other_list = (OtherElem *) myrealloc(
	...
	other->other_data = (OtherData **) myalloc(
...
void *get_new_props_ply(PlyFile *ply) {
	...
	for (i = 0; i < elem->nprops; i++) {
		...
		for (j = 0; j < rules->nprops; j++) {
			...
			get_stored_item((void *) ptr, type, &int_val, &uint_val,
					&double_val);
			vals[j] = double_val;
		}
