...
NVGcontext* nvgCreateInternal(NVGparams* params) {
	...
	ctx->fontImages[0] = ctx->params.renderCreateTexture(ctx->params.userPtr,
			NVG_TEXTURE_ALPHA, fontParams.width, fontParams.height, 0, NULL);
...
int nvgCreateImageRGBA(NVGcontext* ctx, int w, int h, int imageFlags,
	...
	return ctx->params.renderCreateTexture(ctx->params.userPtr,
			NVG_TEXTURE_RGBA, w, h, imageFlags, data);
}
...
static void nvg__tesselateBezier(NVGcontext* ctx, float x1, float y1, float x2,
		float y2, float x3, float y3, float x4, float y4, int level, int type) {
		...
		case NVG_BEZIERTO:
			...
			if (last != NULL) {
				...
				nvg__tesselateBezier(ctx, last->x, last->y, cp1[0], cp1[1],
						cp2[0], cp2[1], p[0], p[1], 0, NVG_PT_CORNER);
			}
			i += 7;
			break;
		case NVG_CLOSE:
...
void nvgQuadTo(NVGcontext* ctx, float cx, float cy, float x, float y) {
	...
	float vals[] = { NVG_BEZIERTO, x0 + 2.0f / 3.0f * (cx - x0), y0
			+ 2.0f / 3.0f * (cy - y0), x + 2.0f / 3.0f * (cx - x), y
			+ 2.0f / 3.0f * (cy - y), x, y };
	nvg__appendCommands(ctx, vals, NVG_COUNTOF(vals));
}
...
void nvgArcTo(NVGcontext* ctx, float x1, float y1, float x2, float y2,
	...
	if (nvg__ptEquals(x0, y0, x1, y1, ctx->distTol)
			...
			|| nvg__distPtSeg(x1, y1, x0, y0, x2, y2)
					< ctx->distTol * ctx->distTol || radius < ctx->distTol) {
		nvgLineTo(ctx, x1, y1);
		return;
	}
...
void nvgArc(NVGcontext* ctx, float cx, float cy, float r, float a0, float a1,
	...
	ndivs = nvg__maxi(1,
			nvg__mini((int) (nvg__absf(da) / (NVG_PI * 0.5f) + 0.5f), 5));
...
void nvgRect(NVGcontext* ctx, float x, float y, float w, float h) {
	float vals[] = { NVG_MOVETO, x, y, NVG_LINETO, x, y + h, NVG_LINETO, x + w,
			y + h, NVG_LINETO, x + w, y, NVG_CLOSE };
	nvg__appendCommands(ctx, vals, NVG_COUNTOF(vals));
}
...
void nvgRoundedRect(NVGcontext* ctx, float x, float y, float w, float h,
	...
	if (r < 0.1f) {
		...
		float rx = nvg__minf(r, nvg__absf(w) * 0.5f) * nvg__signf(w), ry =
				nvg__minf(r, nvg__absf(h) * 0.5f) * nvg__signf(h);
		float vals[] = { NVG_MOVETO, x, y + ry, NVG_LINETO, x, y + h - ry,
				NVG_BEZIERTO, x, y + h - ry * (1 - NVG_KAPPA90), x
						+ rx * (1 - NVG_KAPPA90), y + h, x + rx, y + h,
				NVG_LINETO, x + w - rx, y + h, NVG_BEZIERTO, x + w
						- rx * (1 - NVG_KAPPA90), y + h, x + w, y + h
						- ry * (1 - NVG_KAPPA90), x + w, y + h - ry, NVG_LINETO,
				x + w, y + ry, NVG_BEZIERTO, x + w, y + ry * (1 - NVG_KAPPA90),
				x + w - rx * (1 - NVG_KAPPA90), y, x + w - rx, y, NVG_LINETO, x
						+ rx, y, NVG_BEZIERTO, x + rx * (1 - NVG_KAPPA90), y, x,
				y + ry * (1 - NVG_KAPPA90), x, y + ry, NVG_CLOSE };
		nvg__appendCommands(ctx, vals, NVG_COUNTOF(vals));
	}
}

void nvgEllipse(NVGcontext* ctx, float cx, float cy, float rx, float ry) {
	float vals[] = { NVG_MOVETO, cx - rx, cy, NVG_BEZIERTO, cx - rx, cy
			+ ry * NVG_KAPPA90, cx - rx * NVG_KAPPA90, cy + ry, cx, cy + ry,
			NVG_BEZIERTO, cx + rx * NVG_KAPPA90, cy + ry, cx + rx, cy
					+ ry * NVG_KAPPA90, cx + rx, cy, NVG_BEZIERTO, cx + rx, cy
					- ry * NVG_KAPPA90, cx + rx * NVG_KAPPA90, cy - ry, cx, cy
					- ry, NVG_BEZIERTO, cx - rx * NVG_KAPPA90, cy - ry, cx - rx,
			cy - ry * NVG_KAPPA90, cx - rx, cy, NVG_CLOSE };
	nvg__appendCommands(ctx, vals, NVG_COUNTOF(vals));
}
...
void nvgFill(NVGcontext* ctx) {
	...
	ctx->params.renderFill(ctx->params.userPtr, &fillPaint, &state->scissor,
			ctx->fringeWidth, ctx->cache->bounds, ctx->cache->paths,
			ctx->cache->npaths);
...
void nvgStroke(NVGcontext* ctx) {
	...
	ctx->params.renderStroke(ctx->params.userPtr, &strokePaint, &state->scissor,
...
static void nvg__renderText(NVGcontext* ctx, NVGvertex* verts, int nverts) {
	...
	ctx->params.renderTriangles(ctx->params.userPtr, &paint, &state->scissor,
			verts, nverts);
...
enum NVGcodepointType {
	NVG_SPACE, NVG_NEWLINE, NVG_CHAR,
};
